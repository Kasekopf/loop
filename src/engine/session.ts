...
import { $item, $items, getFoldGroup, sum } from "libram";
...
function mySessionItemsWrapper(): Map<Item, number> {
  ...
  for (const inventoryFunc of [getInventory, getCloset, getDisplay, getStorage, getFreePulls]) {
...
export class Session {
  ...
  static current(): Session {
    return new Session(
      sum([myMeat, myClosetMeat, myStorageMeat], (f) => f()),
